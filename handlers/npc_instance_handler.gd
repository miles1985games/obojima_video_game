...
#func check_npc_schedule(day, hour, minute):
	#if active_npc != null:
		#if npc_data["schedule"].keys().has(hour):
			#var new_location = npc_data["schedule"][hour]
			#
			#if active_npc.current_location.location_id != new_location:
				#active_npc.targeted_location = new_location
				#active_npc.state_machine.state = "exiting_location"
				#print(active_npc.state_machine.state)
			#else:
				#active_npc.state_machine.state = "entering_location"
